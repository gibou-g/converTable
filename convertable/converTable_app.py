...
if __name__ == '__main__':
    ...
    with st.sidebar:
        ...
        img_size = st.selectbox('Preview size', ['256x256', '512x512', '720x720', '1024x1024'])
        brightness = st.slider('Adjust brigtness', 0, 100)
        sharpness = st.slider('Sharpen', 0, 2)
        threshold_method = st.selectbox('Threshold method', [None, 'Adaptive Mean', 'Adaptive Gaussian'])
        ...
        st.number_input('Number of rows to show in preview (default = 5, max = 20)', 5, 20)
    ...
    if apply:
        with col1:
            if input_file:
                ...
                input_img = ImageEnhance.Brightness(input_img).enhance(factor=brightness/100)
